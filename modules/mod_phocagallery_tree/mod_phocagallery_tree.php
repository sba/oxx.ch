   ...
   {   
	  if ($value->parentid == $id)
	  {
		...
		if (strlen($value->text)>4){
			...
			if (preg_match('/(\d{4})-(\d{2})-(\d{2}) (.*)/', $value->text, $treffer)){
				/*
				...
				$linktooltip = addslashes($treffer[3].'.'.$treffer[2].'.'.$treffer[1].' - '.$treffer[4]);
				$linktext = substr(addslashes($treffer[4]),0, 22);
			} else {
				$linktext = substr(addslashes($value->text),0, 22);
				$linktooltip = addslashes($value->text);
			}
...
function getLink( $id )
{
	...
	$link = JRoute::_('index.php?option=com_phocagallery&view=category&id='. $catid_slug .'&Itemid=53');	
	return ($link);
}
...
if(isset($itemsCategories[0])) {
	$itemId = $itemsCategories[0]->id;
	$linkCategories = JRoute::_('http://www.oxx.ch/diverses/galerie.html');
}
