...
(function($){
	...
	$.jceUtilities.popup = {
		...
		_png : function(el, type){
			...
			switch(type){
				case 'image':
					s = el.src;
					if(/\.(png)$/i.test(s)){
						$(el).attr('src', this.site + 'plugins/system/jceutilities/img/blank.gif').css('filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'' + s + '\'');
					}
					break;
				default:
				case 'css':
					...
					if(/\.(png)$/i.test(s)){
						...
						$(el).css('filter', "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + bg + "')");
					}
					break;
			}
		},
		_zoom : function(el){
			...
			$(el).each(function(){	
				...
				if(child && child.nodeName == 'IMG'){
					...
					$(span).attr('style', $(child).attr('style') || '').appendTo($(el));
					// Remove (deprecated) attributes that may affect layout
					['style', 'align', 'border', 'hspace', 'vspace'].each(function(s){
						$(child).removeAttr(s);
					});
					$(child).appendTo($(span));
					$(zoom).addClass('zoom-image').appendTo($(span));
					
					if($.browser.msie){
						$(zoom).css({'margin-left': - 20, 'margin-top': $(child).height() - 20});
					}else{
						$(zoom).css({'margin-left': $(child).width() - 20, 'margin-top': - 20});
					}
				}else{				
				...
				if(t._ie6() && $.jceUtilities.options.pngfix == 1){
					s = $(zoom).css('background-image');
					if(s && /\.(png)/i.test(s)){
						s = /url\("(.+)"\)/.exec(s)[1];	
					}
					$(zoom).css('background-image', 'none');
					$(zoom).css('filter', "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + s + "')");
				}
			});
		},
		// Public
		...
		_position: function(){
			...
			if(this.options.scrollpopup == 1){
				$(this.container).css({'top': $(window).scrollTop() + 50});
			}
		},	
		_bind: function(open){
			var t = this;
		...
		_queue: function(n){
			...
			if(this.closelink){
				//this.closelink.hide();
				this.closelink.css('visibility', 'hidden');
			}
		...
		_change: function(n){			
			...
			// IFrame / Ajax
				...
				if((this.active.type == 'ajax' || /text\/(xml|html|htm)/i.test(this.active.type)) && this._local(this.active.src) && /\.(php|html|htm|txt)/i.test(this.active.src)){
					...
					if(this._local(this.active.src) && /\.(php|html|htm)/i.test(this.active.src) && !/(tmpl=component)/i.test(this.active.src) ){
						this.active.src += /\?/.test(this.active.src) ? '&tmpl=component' : '?tmpl=component';	
					}	
					...
					// Firefox has an issue with pdf files!!!
					if(!/\.pdf/i.test(this.active.src)){
						$(this.iframe).attr('src', this.active.src);	
					}
				}
			}
			return false;
		...
		_animate : function(){
			...
			// Hide loader
			if(/(image|ajax)/.test(t.active.type)){
				if(t.loader){
					t.loader.hide();
				}
			}
				...
				marginLeft: -t.content.outerWidth()/2
					...
					t.container.animate({
						...
						t.content.fadeIn(fs, function(){															
							...
							// Load pdf
							if(/\.(pdf)$/i.test(t.active.src)){
								$(t.iframe).attr('src', t.active.src);
							}
							if(t.closelink){
								t.closelink.show().css('visibility', 'visible');
							}
						});	
					});
			});
