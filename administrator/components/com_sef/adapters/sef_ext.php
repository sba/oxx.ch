 ...
 * @since		1.5
 */
class JInstallerSefExt extends JObject
{
    /**
	 * Constructor
	 *
    ...
    function install()
    {
        ...
        $this->parent->setPath('extension_root', JPATH_ROOT.DS.'components'.DS.'com_sef'.DS.'sef_ext');
        ...
        */
        ...
        if(!$id) {
            $db->setQuery("INSERT INTO `#__sefexts` (`file`) VALUES(".$db->Quote( basename($this->parent->getPath('manifest')) ).")");
            if (!$db->query()) {
                $this->parent->abort( JText::_('SEF Extension').' '.JText::_('Install').': '.JText::_('SQL Error')." ".$db->stderr(true) );
                return false;
            }
