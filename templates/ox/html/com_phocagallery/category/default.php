...
?>
<link type="text/css" media="screen" rel="stylesheet" href="http://www.oxx.ch/includes/js/colorbox/colorbox.css" />
        
<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js"></script>
<script type="text/javascript" src="http://www.oxx.ch/includes/js/colorbox/jquery.colorbox-min.js"></script>
<script type="text/javascript">
    jQuery.noConflict();
    jQuery(document).ready(function(){
        jQuery("a[rel='gallery']").colorbox({slideshow:true,slideshowSpeed:3000,slideshowAuto:false});
    })
</script>
<?php
// Heading
...
if ( $this->tmpl['displaycatnametitle'] == 1) {
	if ($this->category->title != '') {
		...
		if (preg_match('/(\d{4})-(\d{2})-(\d{2}) (.*)/', $this->category->title, $treffer)){
			/*
			[0] => 2008-02-05 Turbo! Die Rockdisco
			[1] => 2008
			[2] => 02
			[3] => 05
			[4] => Turbo! Die Rockdisco*/

			$heading = addslashes($treffer[3].'.'.$treffer[2].'.'.$treffer[1].' - '.$treffer[4]);
		} else {
				...
				<div class="phocagallery-box-file-first" style="height:<?php echo $imageHeight; ?>px;width:<?php echo $imageWidth; ?>px;">
					<div class="phocagallery-box-file-second">
						<div class="phocagallery-box-file-third">
                            ...
                            <?php 
                            if ($value->type == 2) {?>
                                <a href="<?=str_replace('/thumbs/phoca_thumb_m_','/thumbs/phoca_thumb_l_',$value->linkthumbnailpath)?>" rel="gallery" title="">
                                
                                <?php 
                                echo JHTML::_( 'image.site', $value->linkthumbnailpath, '', '', '', $value->title );
                                ?></a><?php
                            } else {
                                ?><a href="<?=$value->link?>"><?php 
                                echo '<img src="'.$value->linkthumbnailpath.'" title="'.$value->title.'" />';
                                ?></a><?php
                            }
                            ?>
