...
class ContentModelCategory extends JModel
{
	...
	function _buildContentWhere($state = 1)
	{
		...
		{
			/*
			* OX Preview - show only current Events
			*/	
			if ($params->get('only_preview')) {
				//$where .= ' AND a.sectionid = 5';
			 } else {
		...
		/*
		 * OX Preview - show only current Events
		 * special OX-filter to fetch multiday-events (with custom MySQL Function 'DIGITS') and show events until 02:00 in night
		 */	
		if ($params->get('only_preview'))
		{
