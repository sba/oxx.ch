	...
	//	$catid = $this->_db->loadObject();
		...
		if ($foto_count > 1)//there are two or more photos, show pause and play icons
		{
			...
			else
			{
				...
				$js_slideshow_data['icons'] = '<a href="'.JRoute::_('index.php?option=com_phocagallery&view=detail&catid='.$catid_slug.'&id='.$id_slug.'&tmpl=component&phocaslideshow=1'.'&Itemid='. JRequest::getVar('Itemid', 1, 'get', 'int')).'" title="'.JText::_( 'Start slideshow' ).'">'
				. JHTML::_('image', 'components/com_phocagallery/assets/images/icon-play.' . $icon, JText::_( 'Start slideshow' )).'</a>';			}
		}
		else
		{
